...
HCSR04::HCSR04(uint8_t trigger_pin, uint8_t echo_pin, int max_cm_distance) {
...
float HCSR04::ping_cm() {
	unsigned int echoTime = HCSR04::ping();          //Calls the ping method and returns with the ping echo distance in uS.
	return HCSR04Convert(echoTime, US_ROUNDTRIP_CM); //Convert uS to centimeters.
}
...
float HCSR04::convert_cm(unsigned int echoTime) {
	return HCSR04Convert(echoTime, US_ROUNDTRIP_CM); //Convert uS to centimeters.
}
